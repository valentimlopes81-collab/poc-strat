...
| — | 25/09 | (Long) | Reação no POC-hoje/VWAP — **miss por velocidade** | POC hoje + VWAP | ⏸️ | — | $0 | **🤖 Mecânico: CONFIRMADO — bateu o TP** (o preço foi mesmo até ao alvo). **🧠 Discricionário:** viu a play, mas **a reação foi demasiado rápida para entrar a tempo** — não foi antecipação nem hesitação de leitura, foi velocidade de clique manual. Custo real de oportunidade (não $, mas dado valioso): confirma que o setup intradiário POC+VWAP continua a validar-se. Nota a levar: considerar ordem limit pré-colocada perto do POI quando a confluência é forte, para não depender só da velocidade de clique |
