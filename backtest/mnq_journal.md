...
### Estado
- **P&L: ~−$215,4** · Balance: **~$24.784,6** · Target: falta **~$1.715,4** de $1.500 · DD: intacto (folga larga)
- Trades live (SISTEMA): 8 (1W/6L/1BE) + no-plays
...
| L8 | 22/09 | (1 MNQ, gatilho/direção não especificados) | Dia de ATH/range após move forte de sexta + one-side move de segunda | PD POC/VAH/VAL | ❌ Loss | −1R | **−$127** | **Plano pré-mkt com 2 cenários bem definidos** (rejeição do VAH → short p/ VAL; OU POC-suporte → teste ao Monday high/ATH) — processo continua sólido. Mas **PA de ATH** (chop nos highs, sem ritmo comprador) → jogou e perdeu. "Não dá para jogar literalmente nada" **depois** da loss — sinal de dia de baixa prob forçado. Fechou os charts pela regra (1 loss = out) ✓. ⚠️ Padrão recorrente: dias de ATH/chop continuam a ser o regime onde mais se força trade sem edge real — reforçar o filtro "sem gatilho confirmado = sem trade", mesmo com plano bem escrito |
