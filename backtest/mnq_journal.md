...
| — | 25/05 | — | — | — | — | — | — | — | ⏸️ No-play | — | de novo em **ATH, muito longe do PD value** (abaixo). **Low-prob day** típico (o preço não volta ao value) → raramente há play nestes dias. Sem setup, sem trade. Disciplina ✓ |
...
- Padrão a cortar: **2 trades administrativas de inatividade** já custaram **~$175** (L5 −$50 provável + L7 −$125 confirmada) sem serem setups reais — usar ping-trade mínimo em vez de risco normal para isto
...
| L3 | 04/09 | Long (1 MNQ) | VAH como suporte (reação **sem volume**) | PD VAH | ❌ Loss | ~−0.5R | **−$80** | Dia de **NFP quente** (hawkish → viés bearish). Tentou o long no VAH-suporte; **reação inicial boa mas faltou volume** → não vingou → −$80. ✅ **Grande salto psicológico:** aceitou a loss sem drama, diagnosticou que "as condições não estavam todas reunidas", bom risk management, e **fechou os charts pela regra (1 loss = out)**. "Hoje senti que estou com melhor cabeça." A execução ainda a afinar (3ª tentativa do long-no-value esta semana — refinar: reação **com volume**, não só reação), mas a **gestão e a mentalidade foram A** |
| — | 08/09 | (Short) | Continuação bearish (rejeição do value) | value | ⏸️ | — | $0 | Reabertura pós-bank holiday (07/09 fechado). **Havia um setup short limpo**, alinhado com o viés bearish desde o NFP — mas **estava fora de casa e não jogou.** Miss sem erro (ausência, não hesitação/tilt). $0, DD intacto. ⚠️ 2º clean setup perdido por **não estar ao ecrã** (04/09 manhã + hoje) — nota logística: o edge só funciona se estiver presente na killzone |
...
| L7 | 18/09 | Short (1 MNQ) | Bias short **correta**, mas entrada apressada pelo prazo de inatividade | — | ❌ Loss | −1R | **−$125** | **A bias (short) estava certa** — não foi um tiro às cegas. O que falhou foi o **timing/execução**: a pressão do contador de inatividade encurtou a paciência para esperar a confirmação limpa, e entrou cedo/mal colocado. Padrão a vigiar: **um prazo administrativo não deve comprimir a qualidade da entrada** — se o setup ainda não tinha confirmado, era ping-trade mínimo (sem risco real) em vez de forçar a entrada real dentro da bias certa mas sem o gatilho pronto |
| L8 | 22/09 | (1 MNQ) | **Leak nº1 — antecipação** (auto-admitido) num dos 2 cenários pré-mkt | PD POC/VAH/VAL | ❌ Loss | −1R | **−$127** | Plano pré-mkt com 2 cenários bem definidos (rejeição VAH→short p/ VAL; POC-suporte→teste Monday high/ATH) — o mapa estava bom. Mas a entrada foi **antecipação, não confirmação** — ele próprio: "foi antecipação errada minha". **É o leak nº1 clássico do backtest** (T6/T8/T10/T16/T18/T24/T40: 7 tentativas, 0 wins, −5R) a reaparecer em live. Fechou os charts pela regra ✓ — gestão ok, mas o gatilho é o ponto a reforçar: PD POC/VAH têm de **fechar** confirmados antes de entrar, não "parecer que vão segurar" |
| L9 | 23/09 | Long (1 MNQ) | **Reação CONFIRMADA no POC de segunda** (segurou bem como suporte) | POC (2ª feira) | ✅ Win | +R | **+$69,9** | Depois da queda forte de ontem, quase não ia trocar (pós-loss) — mas reconheceu uma **oportunidade real**, não forçou uma para "recuperar". O POC de 2ª **funcionou bem como suporte com reação visível** (ao contrário do L8, aqui esperou confirmar). Trail do SL para o PDL a proteger o lucro à medida que subia. Fechou perto das 16h por sentir o **volume a secar** — bom timing de saída, não ganancioso. Win modesto mas **exatamente o contraste que faltava com o L8**: confirmação em vez de antecipação |
| — | 24/09 | (Short) | Rejeição do PD value → LOD | PD value | ⏸️ | — | $0 | Queria rejeição do PD value p/ short até ao LOD, mas **não foi revisitado**. Já passava das 16h → **"vou estar quieto e follow the rules"** — sem gatilho, sem trade, disciplina limpa. Exatamente a lição de 22/09 aplicada: sem confirmação, sem entrada, custe o que custar à vontade de trocar |
| — | 25/09 | (Long) | Reação no POC-hoje/VWAP — **miss por velocidade** | POC hoje + VWAP | ⏸️ | — | $0 | **🤖 Mecânico:** provavelmente teria apanhado (era zona de confluência real, PD value não estava perto mas o setup intradiário existia). **🧠 Discricionário:** viu a play, mas **a reação foi demasiado rápida para entrar a tempo** — não foi antecipação nem hesitação de leitura, foi velocidade de clique manual. Ficaria BE no momento em que reportou; TP incerto. Nota diferente das outras: não é leak de disciplina, é limitação de execução manual em reações muito rápidas — considerar ordem limit pré-colocada perto do POI para estes casos |
